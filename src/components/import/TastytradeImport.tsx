...
export function TastytradeImport({ userId, existingTrades, onImported }: Props) {
  ...
  async function parseCSV(text: string): Promise<{ trades: ParsedTrade[]; carriedForward: { symbol: string; side: string; qty: number }[] }> {
    ...
    rows.forEach(row => {
      ...
      const value  = valueCol  >= 0 ? parseFloat(cols[valueCol])  || 0 : 0
      ...
      const key = orderId ? `${symbol}-${orderId}` : `${symbol}-${dateKey}`

      if (!groups[key]) {
        groups[key] = { symbol, date: dateStr, legs: [] }
      }

      groups[key].legs.push({
        action,
        qty,
        price,
        value,
        date: dateStr,
        commission: comm + fees,
      })
    })
  ...
  return (
    <div style={{ maxWidth: '600px' }}>
      ...
      >
        ...
        <div style={{ fontSize: '13px', fontWeight: 600 }}>Click to upload Tastytrade export file</div>
