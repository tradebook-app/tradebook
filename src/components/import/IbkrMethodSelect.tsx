...
import { IbkrAutoSync } from './IbkrAutoSync'
...
export function IbkrMethodSelect({ userId, existingTrades, onImported }: Props) {
  ...
  if (method === 'sync') {
    return (
      <div>
        ...
        <IbkrAutoSync userId={userId} existingTrades={existingTrades} onImported={onImported} />
      </div>
    )
  }
  ...
  const optionCard = (active: boolean): React.CSSProperties => ({
    ...
    border: `1px solid ${active ? '#10B981' : 'var(--brd)'}`, borderRadius: '12px',
  ...
  return (
    <div style={{ maxWidth: '640px', margin: '0 auto' }}>
      <div style={{ textAlign: 'center', marginBottom: '8px' }}>
        ...
        <div style={{ fontSize: '13px', color: 'var(--txt3)', marginBottom: '28px' }}>You're linking Interactive Brokers</div>
      </div>

      <div style={{ display: 'flex', gap: '14px', flexWrap: 'wrap' }}>
        <div
          onClick={() => setMethod('sync')}
          style={optionCard(false)}
        ...
        >
          ...
          <div style={{ fontSize: '11px', color: 'var(--txt3)' }}>Connect your broker — trades sync automatically</div>
        </div>
        ...
        >
          ...
          <div style={{ fontSize: '11px', color: 'var(--txt3)' }}>Upload your IBKR Activity Statement CSV</div>
        </div>
      </div>
    </div>
  )
