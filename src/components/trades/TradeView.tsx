...
export function TradeView({ trades, filter, onEdit, onDelete, onDeleteFiltered }: Props) {
  ...
  return (
    <>
      ...
      <div style={{ display: 'flex', gap: '6px', marginBottom: '12px', flexWrap: 'wrap', alignItems: 'center' }}>
        <input style={{ ...finput, width: '80px', textTransform: 'uppercase' }}
          placeholder="Symbol..." value={symFilter} onChange={e => setSymFilter(e.target.value)} />
        ...
        <button
          ...
          style={{
            ...
            fontSize: '11px', fontWeight: 600, cursor: 'pointer', fontFamily: 'var(--sans)',
          }}
        >🗑 Delete All</button>
      </div>
  ...
  {isW ? 'WIN' : isL ? 'LOSS' : 'BE'}
                  ...
                  <td className="r" style={{ fontFamily: 'var(--mono)', color: isW ? 'var(--ac)' : isL ? 'var(--red)' : '', fontWeight: 600 }}>
                    {fmtPnl(t.pnl)}
                  </td>
                  <td className="r" style={{ fontFamily: 'var(--mono)', fontSize: '10px', color: roi >= 0 ? 'var(--ac)' : 'var(--red)' }}>
                    {roi.toFixed(2)}%
                  </td>
                  <td className="r" style={{ fontFamily: 'var(--mono)', fontSize: '10px' }}>
                    {rm !== null ? `${rm.toFixed(2)}R` : '—'}
                  </td>
                  ...
                  <td>
                    <button
                      className="btn-d"
                      onClick={e => { e.stopPropagation(); if (confirm('Delete?')) onDelete(t.id) }}
                      style={{ padding: '3px 8px', fontSize: '10px', borderRadius: '4px', cursor: 'pointer' }}
                    >✕</button>
                  </td>
                </tr>
              )
