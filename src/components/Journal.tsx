...
function PnlBadge({ pnl }: { pnl: number }) {
  const color = pnl > 0 ? 'var(--ac)' : pnl < 0 ? 'var(--red)' : 'var(--txt3)'
  return <span style={{ color, fontWeight: 700, fontFamily: 'var(--mono)' }}>{pnl >= 0 ? '+' : ''}{pnl.toFixed(2)}</span>
}
        ...
        {mode === 'day' && (
          <>
            ...
            <div style={{ background: 'var(--bg2)', border: '1px solid var(--brd)', borderRadius: 'var(--r2)', padding: '16px 20px' }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: dayTrades.length > 0 ? '16px' : '0' }}>
                <div>
                  ...
                  {dayTrades.length > 0 && (
                    <div style={{ fontSize: '13px' }}>
                      Net P&L: <PnlBadge pnl={dayStats.netPnl} />
                    </div>
                  )}
                </div>
                <div style={{ display: 'flex', gap: '8px' }}>
                  ...
                  }} style={{ background: 'var(--bg3)', border: '1px solid var(--brd)', borderRadius: 'var(--r)', padding: '5px 12px', color: 'var(--txt2)', fontSize: '12px', cursor: 'pointer', fontFamily: 'var(--sans)' }}>← Prev</button>
                  <button onClick={() => setSelectedDate(today)} style={{ background: 'var(--bg3)', border: '1px solid var(--brd)', borderRadius: 'var(--r)', padding: '5px 12px', color: 'var(--txt2)', fontSize: '12px', cursor: 'pointer', fontFamily: 'var(--sans)' }}>Today</button>
                  <button onClick={() => {
                    const d = new Date(selectedDate)
                    d.setDate(d.getDate() + 1)
                    setSelectedDate(formatDate(d))
                  }} style={{ background: 'var(--bg3)', border: '1px solid var(--brd)', borderRadius: 'var(--r)', padding: '5px 12px', color: 'var(--txt2)', fontSize: '12px', cursor: 'pointer', fontFamily: 'var(--sans)' }}>Next →</button>
                </div>
              </div>
              ...
              {dayTrades.length === 0 && (
                <div style={{ textAlign: 'center', padding: '32px 0', color: 'var(--txt3)', fontSize: '12px' }}>
            ...
            {dayTrades.length > 0 && (
              <div style={{ background: 'var(--bg2)', border: '1px solid var(--brd)', borderRadius: 'var(--r2)', overflow: 'hidden' }}>
                <table className="tbl" style={{ width: '100%' }}>
                  ...
                  <tbody>
                    {dayTrades.sort((a, b) => a.date.localeCompare(b.date)).map(t => (
                      <tr key={t.id} onClick={() => onEdit(t)} style={{ cursor: 'pointer' }}>
                        ...
                        <td>{t.grade ? <span style={{ fontSize: '11px', fontWeight: 700, color: t.grade === 'A' ? 'var(--ac)' : t.grade === 'B' ? 'var(--blue)' : t.grade === 'C' ? 'var(--orange)' : 'var(--red)' }}>{t.grade}</span> : '—'}</td>
            ...
            <div style={{ background: 'var(--bg2)', border: '1px solid var(--brd)', borderRadius: 'var(--r2)', padding: '16px 20px' }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '16px' }}>
                <div>
                  ...
                  {weekTrades.length > 0 && (
                    <div style={{ fontSize: '13px' }}>Net P&L: <PnlBadge pnl={weekStats.netPnl} /></div>
                  )}
                </div>
                <div style={{ display: 'flex', gap: '8px' }}>
                  <button onClick={() => {
                  ...
                  <button onClick={() => setSelectedDate(today)} style={{ background: 'var(--bg3)', border: '1px solid var(--brd)', borderRadius: 'var(--r)', padding: '5px 12px', color: 'var(--txt2)', fontSize: '12px', cursor: 'pointer', fontFamily: 'var(--sans)' }}>This Week</button>
                  <button onClick={() => {
                    const d = new Date(selectedDate)
                    d.setDate(d.getDate() + 7)
                    setSelectedDate(formatDate(d))
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)', gap: '6px', marginBottom: '16px' }}>
                {weekDays.map(d => {
                  ...
                  return (
                    <div key={d} onClick={() => { setSelectedDate(d); setMode('day') }} style={{
                      background: hasTrades ? (pnl >= 0 ? 'rgba(16,185,129,.08)' : 'rgba(239,68,68,.08)') : 'var(--bg3)',
                      border: `1px solid ${isSelected ? 'var(--ac)' : isToday ? 'var(--brd2)' : 'var(--brd)'}`,
                      borderRadius: 'var(--r)', padding: '10px 8px', textAlign: 'center', cursor: 'pointer',
                      transition: '.15s',
                    }}>
                      <div style={{ fontSize: '9px', color: 'var(--txt3)', textTransform: 'uppercase', letterSpacing: '.06em', marginBottom: '4px' }}>
                        {new Date(d + 'T12:00:00').toLocaleDateString('en-US', { weekday: 'short' })}
                      </div>
                      <div style={{ fontSize: '11px', color: 'var(--txt2)', marginBottom: '4px' }}>
                        {new Date(d + 'T12:00:00').getDate()}
                      </div>
                      {hasTrades ? (
                        <>
                          <div style={{ fontSize: '11px', fontWeight: 700, color: pnl >= 0 ? 'var(--ac)' : 'var(--red)', fontFamily: 'var(--mono)' }}>
                            {pnl >= 0 ? '+' : ''}{pnl.toFixed(0)}
                          </div>
              ...
              {weekTrades.length === 0 && (
                <div style={{ textAlign: 'center', padding: '32px 0', color: 'var(--txt3)', fontSize: '12px' }}>No trades this week</div>
              )}
            </div>
                ...
                <table className="tbl" style={{ width: '100%' }}>
                  ...
                  <tbody>
                    {weekTrades.sort((a, b) => a.date.localeCompare(b.date)).map(t => (
                      <tr key={t.id} onClick={() => onEdit(t)} style={{ cursor: 'pointer' }}>
                        <td style={{ color: 'var(--txt3)' }}>{new Date(t.date).toLocaleDateString('en-US', { weekday: 'short', month: 'short', day: 'numeric' })}</td>
                        <td style={{ color: 'var(--txt3)' }}>{new Date(t.date).toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit' })}</td>
                        <td style={{ fontWeight: 700 }}>{t.symbol}</td>
                        <td><span className={t.type === 'Long' ? 'badge badge-long' : 'badge badge-short'}>{t.type}</span></td>
                        <td style={{ fontFamily: 'var(--mono)' }}>${t.entry.toFixed(2)}</td>
      ...
      <div style={{ width: '220px', flexShrink: 0 }}>
        <div style={{ background: 'var(--bg2)', border: '1px solid var(--brd)', borderRadius: 'var(--r2)', padding: '14px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)', gap: '2px', marginBottom: '4px' }}>
            {DAYS_SHORT.map(d => (
              <div key={d} style={{ fontSize: '8px', color: 'var(--txt4)', textAlign: 'center', fontWeight: 600 }}>{d}</div>
            ))}
          </div>
            ...
            {Array.from({ length: daysInMonth }).map((_, i) => {
              ...
              return (
                <div key={day} onClick={() => selectDay(day)} style={{
                  ...
                  fontSize: '10px', fontWeight: isSelected || isToday ? 700 : 400,
                  borderRadius: '5px', cursor: 'pointer', transition: '.1s',
                  background: isSelected ? 'var(--ac)' : hasTrades ? (pnl >= 0 ? 'rgba(16,185,129,.15)' : 'rgba(239,68,68,.15)') : 'transparent',
                  color: isSelected ? '#000' : hasTrades ? (pnl >= 0 ? 'var(--ac)' : 'var(--red)') : isToday ? 'var(--txt)' : 'var(--txt2)',
                  border: isToday && !isSelected ? '1px solid var(--brd2)' : '1px solid transparent',
                }}>
                  {day}
                </div>
              )
          ...
          <div style={{ marginTop: '12px', paddingTop: '12px', borderTop: '1px solid var(--brd)', display: 'flex', gap: '10px', justifyContent: 'center' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '4px', fontSize: '9px', color: 'var(--txt3)' }}>
              <div style={{ width: '8px', height: '8px', borderRadius: '2px', background: 'rgba(16,185,129,.3)' }} /> Win day
            </div>
            <div style={{ display: 'flex', alignItems: 'center', gap: '4px', fontSize: '9px', color: 'var(--txt3)' }}>
              <div style={{ width: '8px', height: '8px', borderRadius: '2px', background: 'rgba(239,68,68,.3)' }} /> Loss day
            </div>
          </div>
        </div>
      </div>
